...
function asset_allotment_table_insert(&$error_message = '') {
	...
	// automatic asset_details if passed as filterer
	if(Request::val('filterer_asset_details')) {
		$_REQUEST['asset_details'] = Request::val('filterer_asset_details');
	}
	...
	// automatic asset_details if passed as filterer
	if(Request::val('filterer_asset_details')) {
		$data['asset_details'] = Request::val('filterer_asset_details');
	}
...
function asset_allotment_table_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	$filterer_asset_details = Request::val('filterer_asset_details');
	...
	// combobox: asset_details
	$combo_asset_details = new DataCombo;
	...
	if($hasSelectedId) {
		...
		$combo_asset_details->SelectedData = $row['asset_details'];
		...
		$combo_asset_details->SelectedData = $filterer_asset_details;
	...
	$combo_asset_details->HTML = '<span id="asset_details-container' . $rnd1 . '"></span><input type="hidden" name="asset_details" id="asset_details' . $rnd1 . '" value="' . html_attr($combo_asset_details->SelectedData) . '">';
	$combo_asset_details->MatchText = '<span id="asset_details-container-readonly' . $rnd1 . '"></span><input type="hidden" name="asset_details" id="asset_details' . $rnd1 . '" value="' . html_attr($combo_asset_details->SelectedData) . '">';
	...
	<script>
		...
		AppGini.current_asset_details__RAND__ = { text: "", value: "<?php echo addslashes($hasSelectedId ? $urow['asset_details'] : htmlspecialchars($filterer_asset_details, ENT_QUOTES)); ?>"};
		...
		$j(function() {
			setTimeout(function() {
				if(typeof(asset_details_reload__RAND__) == 'function') asset_details_reload__RAND__();
		...
		function asset_details_reload__RAND__() {
		<?php if($fieldsAreEditable) { ?>

			$j("#asset_details-container__RAND__").select2({
				/* initial default value */
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_asset_details__RAND__.value, t: 'asset_allotment_table', f: 'asset_details' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
							});
							$j('[name="asset_details"]').val(resp.results[0].id);
							$j('[id=asset_details-container-readonly__RAND__]').html('<span class="match-text" id="asset_details-match-text">' + resp.results[0].text + '</span>');
							...
							if(typeof(asset_details_update_autofills__RAND__) == 'function') asset_details_update_autofills__RAND__();
						}
					});
				},
				width: '100%',
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'asset_allotment_table', f: 'asset_details' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
				AppGini.current_asset_details__RAND__.value = e.added.id;
				AppGini.current_asset_details__RAND__.text = e.added.text;
				$j('[name="asset_details"]').val(e.added.id);
			...
			if(!$j("#asset_details-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_asset_details__RAND__.value, t: 'asset_allotment_table', f: 'asset_details' },
		...
		<?php } else { ?>
			...
			$j.ajax({
				...
				data: { id: AppGini.current_asset_details__RAND__.value, t: 'asset_allotment_table', f: 'asset_details' },
				success: function(resp) {
					$j('[id=asset_details-container__RAND__], [id=asset_details-container-readonly__RAND__]').html('<span class="match-text" id="asset_details-match-text">' + resp.results[0].text + '</span>');
					if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=asset_table_view_parent]').hide(); } else { $j('.btn[id=asset_table_view_parent]').show(); }
	...
	// process combos
	$templateCode = str_replace('<%%COMBO(asset_details)%%>', $combo_asset_details->HTML, $templateCode);
	$templateCode = str_replace('<%%COMBOTEXT(asset_details)%%>', $combo_asset_details->MatchText, $templateCode);
	$templateCode = str_replace('<%%URLCOMBOTEXT(asset_details)%%>', urlencode($combo_asset_details->MatchText), $templateCode);
	...
	$lookup_fields = ['asset_details' => ['asset_table', 'Asset Details'], 'select_employee' => ['user_table', 'Select employee'], 'alloted_by' => ['user_table', 'Alloted by'], ];
	...
	$templateCode = str_replace('<%%UPLOADFILE(asset_details)%%>', '', $templateCode);
	...
	if($hasSelectedId) {
		...
		$templateCode = str_replace('<%%VALUE(asset_details)%%>', safe_html($urow['asset_details']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(asset_details)%%>', urlencode($urow['asset_details']), $templateCode);
		...
		$templateCode = str_replace('<%%VALUE(asset_details)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(asset_details)%%>', urlencode(''), $templateCode);
	...
	if(isset($filterField[1]) && $filterField[1] == '2' && $filterOperator[1] == '<=>')
		$templateCode.="\n<input type=hidden name=asset_details value=\"" . html_attr($filterValue[1]) . "\">\n";
