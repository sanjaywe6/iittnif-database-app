...
// Data functions (insert, update, delete, form) for table beyond_workingHours_table

// This script and data application was generated by AppGini, https://bigprof.com/appgini
// Download AppGini for free from https://bigprof.com/appgini/download/

function beyond_workingHours_table_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('beyond_workingHours_table');
	if(!$arrPerm['insert']) {
	...
	$recID = tableInsert('beyond_workingHours_table', $data, $recordOwner, $error_message);

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID')) && $recID !== false)
		beyond_workingHours_table_copy_children($recID, Request::val('SelectedID'));

	return $recID;
}

function beyond_workingHours_table_copy_children($destination_id, $source_id) {
	global $Translation;
...
function beyond_workingHours_table_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('beyond_workingHours_table', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: beyond_workingHours_table_before_delete
	if(function_exists('beyond_workingHours_table_before_delete')) {
		$args = [];
		if(!beyond_workingHours_table_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	sql("DELETE FROM `beyond_workingHours_table` WHERE `id`='{$selected_id}'", $eo);

	// hook: beyond_workingHours_table_after_delete
	if(function_exists('beyond_workingHours_table_after_delete')) {
		$args = [];
		beyond_workingHours_table_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='beyond_workingHours_table' AND `pkValue`='{$selected_id}'", $eo);
}

function beyond_workingHours_table_update(&$selected_id, &$error_message = '') {
	global $Translation;
	...
	// hook: beyond_workingHours_table_before_update
	if(function_exists('beyond_workingHours_table_before_update')) {
		$args = ['old_data' => $old_data];
		if(!beyond_workingHours_table_before_update($data, getMemberInfo(), $args)) {
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="beyond_workingHours_table_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}


	update_calc_fields('beyond_workingHours_table', $data['selectedID'], calculated_fields()['beyond_workingHours_table']);

	// hook: beyond_workingHours_table_after_update
	if(function_exists('beyond_workingHours_table_after_update')) {
		if($row = getRecord('beyond_workingHours_table', $data['selectedID'])) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['id'];
		$args = ['old_data' => $old_data];
		if(!beyond_workingHours_table_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update record update timestamp
	set_record_owner('beyond_workingHours_table', $selected_id);
}

function beyond_workingHours_table_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	$allowUpdate = $hasSelectedId && check_record_permission('beyond_workingHours_table', $selectedId, 'edit');
	$allowDelete = $hasSelectedId && check_record_permission('beyond_workingHours_table', $selectedId, 'delete');
	...
	if($hasSelectedId && !check_record_permission('beyond_workingHours_table', $selectedId, 'view'))
	...
	if(is_file(__DIR__ . '/hooks/beyond_workingHours_table.approval_status.csv')) {
		$approval_status_data = addslashes(implode('', @file(__DIR__ . '/hooks/beyond_workingHours_table.approval_status.csv')));
			...
			$j("#emp_lookup-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_emp_lookup__RAND__.value, t: 'beyond_workingHours_table', f: 'emp_lookup' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'beyond_workingHours_table', f: 'emp_lookup' }; },
	...
	if($dvprint) {
		$template_file = is_file("./{$templateDVP}") ? "./{$templateDVP}" : './templates/beyond_workingHours_table_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$templateDV}") ? "./{$templateDV}" : './templates/beyond_workingHours_table_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	$rdata = $jdata = get_defaults('beyond_workingHours_table');
	if($hasSelectedId) {
		$jdata = get_joined_record('beyond_workingHours_table', $selectedId);
		if($jdata === false) $jdata = get_defaults('beyond_workingHours_table');
		$rdata = $row;
	}
	$templateCode .= loadView('beyond_workingHours_table-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);

	// hook: beyond_workingHours_table_dv
	if(function_exists('beyond_workingHours_table_dv')) {
		$args = [];
		beyond_workingHours_table_dv(($hasSelectedId ? $selectedId : FALSE), getMemberInfo(), $templateCode, $args);
	}
