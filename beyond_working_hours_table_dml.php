...
function beyond_working_hours_table_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	if(is_file(__DIR__ . '/hooks/beyond_working_hours_table.approval_status.csv')) {
		...
		$combo_approval_status->ListItem = array_trim(explode('||', entitiesToUTF8(convertLegacyOptions("Approved;;Not Approved"))));
		$combo_approval_status->ListData = $combo_approval_status->ListItem;
	}
	...
	if($hasSelectedId) {
		...
		$combo_approval_status->SelectedText = (isset($filterField[1]) && $filterField[1] == '7' && $filterOperator[1] == '<=>' ? $filterValue[1] : entitiesToUTF8(''));
	}
			...
			$j("#select_employee-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_select_employee__RAND__.value, t: 'beyond_working_hours_table', f: 'select_employee' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'beyond_working_hours_table', f: 'select_employee' }; },
