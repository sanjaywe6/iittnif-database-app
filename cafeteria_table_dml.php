...
// Data functions (insert, update, delete, form) for table cafeteria_table

// This script and data application was generated by AppGini, https://bigprof.com/appgini
// Download AppGini for free from https://bigprof.com/appgini/download/

function cafeteria_table_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('cafeteria_table');
	if(!$arrPerm['insert']) {
	...
	$data = [
		'type' => Request::val('type', 'Breakfast'),
	...
	$recID = tableInsert('cafeteria_table', $data, $recordOwner, $error_message);

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID')) && $recID !== false)
		cafeteria_table_copy_children($recID, Request::val('SelectedID'));

	return $recID;
}

function cafeteria_table_copy_children($destination_id, $source_id) {
	global $Translation;
...
function cafeteria_table_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('cafeteria_table', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: cafeteria_table_before_delete
	if(function_exists('cafeteria_table_before_delete')) {
		$args = [];
		if(!cafeteria_table_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	sql("DELETE FROM `cafeteria_table` WHERE `id`='{$selected_id}'", $eo);

	// hook: cafeteria_table_after_delete
	if(function_exists('cafeteria_table_after_delete')) {
		$args = [];
		cafeteria_table_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='cafeteria_table' AND `pkValue`='{$selected_id}'", $eo);
}

function cafeteria_table_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('cafeteria_table', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('cafeteria_table', $selected_id);
	...
	// hook: cafeteria_table_before_update
	if(function_exists('cafeteria_table_before_update')) {
		$args = ['old_data' => $old_data];
		if(!cafeteria_table_before_update($data, getMemberInfo(), $args)) {
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="cafeteria_table_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}


	update_calc_fields('cafeteria_table', $data['selectedID'], calculated_fields()['cafeteria_table']);

	// hook: cafeteria_table_after_update
	if(function_exists('cafeteria_table_after_update')) {
		if($row = getRecord('cafeteria_table', $data['selectedID'])) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['id'];
		$args = ['old_data' => $old_data];
		if(!cafeteria_table_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update record update timestamp
	set_record_owner('cafeteria_table', $selected_id);
}

function cafeteria_table_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	$allowUpdate = $hasSelectedId && check_record_permission('cafeteria_table', $selectedId, 'edit');
	$allowDelete = $hasSelectedId && check_record_permission('cafeteria_table', $selectedId, 'delete');
	...
	if($hasSelectedId && !check_record_permission('cafeteria_table', $selectedId, 'view'))
	...
	if(is_file(__DIR__ . '/hooks/cafeteria_table.type.csv')) {
		$type_data = addslashes(implode('', @file(__DIR__ . '/hooks/cafeteria_table.type.csv')));
		...
		$combo_type->ListItem = array_trim(explode('||', entitiesToUTF8(convertLegacyOptions("Breakfast;;Lunch;;Morning Snack;;Evening Snack;;Dinner"))));
		$combo_type->ListData = $combo_type->ListItem;
	}
	...
	$combo_date->MinYear = defined('cafeteria_table.date.MinYear') ? constant('cafeteria_table.date.MinYear') : 1900;
	$combo_date->MaxYear = defined('cafeteria_table.date.MaxYear') ? constant('cafeteria_table.date.MaxYear') : 2100;
	...
	if($hasSelectedId) {
		if(!($row = getRecord('cafeteria_table', $selectedId))) {
			return error_message($Translation['No records found'], 'cafeteria_table_view.php', false);
		}
		...
		$combo_type->SelectedText = (isset($filterField[1]) && $filterField[1] == '2' && $filterOperator[1] == '<=>' ? $filterValue[1] : entitiesToUTF8('Breakfast'));
	}
	$combo_type->Render();
	...
	if($dvprint) {
		$template_file = is_file("./{$templateDVP}") ? "./{$templateDVP}" : './templates/cafeteria_table_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$templateDV}") ? "./{$templateDV}" : './templates/cafeteria_table_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	if($hasSelectedId) {
		...
		$templateCode = str_replace('<%%VALUE(type)%%>', 'Breakfast', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(type)%%>', urlencode('Breakfast'), $templateCode);
	...
	$rdata = $jdata = get_defaults('cafeteria_table');
	if($hasSelectedId) {
		$jdata = get_joined_record('cafeteria_table', $selectedId);
		if($jdata === false) $jdata = get_defaults('cafeteria_table');
		$rdata = $row;
	}
	$templateCode .= loadView('cafeteria_table-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);

	// hook: cafeteria_table_dv
	if(function_exists('cafeteria_table_dv')) {
		$args = [];
		cafeteria_table_dv(($hasSelectedId ? $selectedId : FALSE), getMemberInfo(), $templateCode, $args);
	}
