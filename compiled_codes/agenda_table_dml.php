...
// Data functions (insert, update, delete, form) for table agenda_table

// This script and data application was generated by AppGini, https://bigprof.com/appgini
// Download AppGini for free from https://bigprof.com/appgini/download/

function agenda_table_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('agenda_table');
	if(!$arrPerm['insert']) {
	...
	$recID = tableInsert('agenda_table', $data, $recordOwner, $error_message);

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID')) && $recID !== false)
		agenda_table_copy_children($recID, Request::val('SelectedID'));

	return $recID;
}

function agenda_table_copy_children($destination_id, $source_id) {
	global $Translation;
...
function agenda_table_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('agenda_table', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: agenda_table_before_delete
	if(function_exists('agenda_table_before_delete')) {
		$args = [];
		if(!agenda_table_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	$res = sql("SELECT `agenda_id` FROM `agenda_table` WHERE `agenda_id`='{$selected_id}'", $eo);
	$agenda_id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `decision_table` WHERE `agenda_lookup`='" . makeSafe($agenda_id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	$childrenATag = '<a class="alert-link" href="decision_table_view.php?filterer_agenda_lookup=' . urlencode($agenda_id[0]) . '">%s</a>';
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		...
		$RetMsg = str_replace('<RelatedRecords>', sprintf($childrenATag, $rirow[0]), $RetMsg);
		$RetMsg = str_replace(['[<TableName>]', '<TableName>'], sprintf($childrenATag, 'decision_table'), $RetMsg);
		$RetMsg = str_replace('<Delete>', '<input type="button" class="btn btn-danger" value="' . html_attr($Translation['yes']) . '" onClick="window.location = \'agenda_table_view.php?SelectedID=' . urlencode($selected_id) . '&delete_x=1&confirmed=1&csrf_token=' . urlencode(csrf_token(false, true)) . '\';">', $RetMsg);
		$RetMsg = str_replace('<Cancel>', '<input type="button" class="btn btn-success" value="' . html_attr($Translation[ 'no']) . '" onClick="window.location = \'agenda_table_view.php?SelectedID=' . urlencode($selected_id) . '\';">', $RetMsg);
		return $RetMsg;
	}

	sql("DELETE FROM `agenda_table` WHERE `agenda_id`='{$selected_id}'", $eo);

	// hook: agenda_table_after_delete
	if(function_exists('agenda_table_after_delete')) {
		$args = [];
		agenda_table_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='agenda_table' AND `pkValue`='{$selected_id}'", $eo);
}

function agenda_table_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('agenda_table', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('agenda_table', $selected_id);
	...
	// hook: agenda_table_before_update
	if(function_exists('agenda_table_before_update')) {
		$args = ['old_data' => $old_data];
		if(!agenda_table_before_update($data, getMemberInfo(), $args)) {
	...
	if(!update(
		...
		['`agenda_id`' => $selected_id], 
		$error_message
	)) {
		echo $error_message;
		echo '<a href="agenda_table_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}


	update_calc_fields('agenda_table', $data['selectedID'], calculated_fields()['agenda_table']);

	// hook: agenda_table_after_update
	if(function_exists('agenda_table_after_update')) {
		if($row = getRecord('agenda_table', $data['selectedID'])) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['agenda_id'];
		$args = ['old_data' => $old_data];
		if(!agenda_table_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update record update timestamp
	set_record_owner('agenda_table', $selected_id);
}

function agenda_table_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	$allowUpdate = $hasSelectedId && check_record_permission('agenda_table', $selectedId, 'edit');
	$allowDelete = $hasSelectedId && check_record_permission('agenda_table', $selectedId, 'delete');
	...
	if($hasSelectedId && !check_record_permission('agenda_table', $selectedId, 'view'))
	...
	if($hasSelectedId) {
		if(!($row = getRecord('agenda_table', $selectedId))) {
			return error_message($Translation['No records found'], 'agenda_table_view.php', false);
		}
			...
			$j("#meeting_lookup-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_meeting_lookup__RAND__.value, t: 'agenda_table', f: 'meeting_lookup' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'agenda_table', f: 'meeting_lookup' }; },
	...
	if($dvprint) {
		$template_file = is_file("./{$templateDVP}") ? "./{$templateDVP}" : './templates/agenda_table_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$templateDV}") ? "./{$templateDV}" : './templates/agenda_table_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	if(!$fieldsAreEditable) {
		$jsReadOnly = '';
		$jsReadOnly .= "\tjQuery('#agenda_description').replaceWith('<div class=\"form-control-static\" id=\"agenda_description\">' + (jQuery('#agenda_description').val() || '') + '</div>');\n";
	...
	if($hasSelectedId) {
		...
		$templateCode = str_replace('<%%VALUE(agenda_str)%%>', safe_html($urow['agenda_str']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(agenda_str)%%>', urlencode($urow['agenda_str']), $templateCode);
		...
		$templateCode = str_replace('<%%VALUE(agenda_str)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(agenda_str)%%>', urlencode(''), $templateCode);
	...
	$rdata = $jdata = get_defaults('agenda_table');
	if($hasSelectedId) {
		$jdata = get_joined_record('agenda_table', $selectedId);
		if($jdata === false) $jdata = get_defaults('agenda_table');
		$rdata = $row;
	}
	$templateCode .= loadView('agenda_table-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);

	// hook: agenda_table_dv
	if(function_exists('agenda_table_dv')) {
		$args = [];
		agenda_table_dv(($hasSelectedId ? $selectedId : FALSE), getMemberInfo(), $templateCode, $args);
	}
