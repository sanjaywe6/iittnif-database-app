...
function asset_allotment_table_insert(&$error_message = '') {
	...
	$data = [
		...
		'employee_id' => Request::val('employee_id', ''),
...
function asset_allotment_table_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	sql("DELETE FROM `asset_allotment_table` WHERE `asset_allotment_id`='{$selected_id}'", $eo);
...
function asset_allotment_table_update(&$selected_id, &$error_message = '') {
	...
	if(!update(
		...
		['`asset_allotment_id`' => $selected_id], 
		$error_message
	)) {
		echo $error_message;
	...
	if(function_exists('asset_allotment_table_after_update')) {
		...
		$data['selectedID'] = $data['asset_allotment_id'];
		$args = ['old_data' => $old_data];
...
function asset_allotment_table_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	if($hasSelectedId) {
		...
		$combo_status->SelectedText = (isset($filterField[1]) && $filterField[1] == '10' && $filterOperator[1] == '<=>' ? $filterValue[1] : entitiesToUTF8(''));
	}
	...
	$lookup_fields = ['asset_lookup' => ['asset_table', 'Asset'], 'alloted_by' => ['user_table', 'Alloted by'], ];
	...
	// process images
	$templateCode = str_replace('<%%UPLOADFILE(asset_allotment_id)%%>', '', $templateCode);
	...
	$templateCode = str_replace('<%%UPLOADFILE(employee_id)%%>', '', $templateCode);
	...
	// process values
	if($hasSelectedId) {
		$templateCode = str_replace('<%%VALUE(asset_allotment_id)%%>', safe_html($urow['asset_allotment_id']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(asset_allotment_id)%%>', urlencode($urow['asset_allotment_id']), $templateCode);
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(employee_id)%%>', safe_html($urow['employee_id']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(employee_id)%%>', html_attr($row['employee_id']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(employee_id)%%>', urlencode($urow['employee_id']), $templateCode);
		...
		$templateCode = str_replace('<%%VALUE(asset_allotment_id)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(asset_allotment_id)%%>', urlencode(''), $templateCode);
		...
		$templateCode = str_replace('<%%VALUE(employee_id)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(employee_id)%%>', urlencode(''), $templateCode);
