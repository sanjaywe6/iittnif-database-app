...
// Data functions (insert, update, delete, form) for table computer_user_details

// This script and data application was generated by AppGini, https://bigprof.com/appgini
// Download AppGini for free from https://bigprof.com/appgini/download/

function computer_user_details_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('computer_user_details');
	if(!$arrPerm['insert']) {
	...
	$recID = tableInsert('computer_user_details', $data, $recordOwner, $error_message);

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID')) && $recID !== false)
		computer_user_details_copy_children($recID, Request::val('SelectedID'));

	return $recID;
}

function computer_user_details_copy_children($destination_id, $source_id) {
	global $Translation;
...
function computer_user_details_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('computer_user_details', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: computer_user_details_before_delete
	if(function_exists('computer_user_details_before_delete')) {
		$args = [];
		if(!computer_user_details_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	sql("DELETE FROM `computer_user_details` WHERE `id`='{$selected_id}'", $eo);

	// hook: computer_user_details_after_delete
	if(function_exists('computer_user_details_after_delete')) {
		$args = [];
		computer_user_details_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='computer_user_details' AND `pkValue`='{$selected_id}'", $eo);
}

function computer_user_details_update(&$selected_id, &$error_message = '') {
	global $Translation;
	...
	if($data['pc_id'] === '') {
		echo StyleSheet() . "\n\n<div class=\"alert alert-danger\">{$Translation['error:']} 'PC ID': {$Translation['field not null']}<br><br>";
		echo '<a href="" onclick="history.go(-1); return false;">' . $Translation['< back'] . '</a></div>';
		exit;
	}
	if($data['entry_time'] === '') {
		echo StyleSheet() . "\n\n<div class=\"alert alert-danger\">{$Translation['error:']} 'Entry time': {$Translation['field not null']}<br><br>";
		echo '<a href="" onclick="history.go(-1); return false;">' . $Translation['< back'] . '</a></div>';
		exit;
	}
	// get existing values
	$old_data = getRecord('computer_user_details', $selected_id);
	...
	// hook: computer_user_details_before_update
	if(function_exists('computer_user_details_before_update')) {
		$args = ['old_data' => $old_data];
		if(!computer_user_details_before_update($data, getMemberInfo(), $args)) {
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="computer_user_details_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}


	update_calc_fields('computer_user_details', $data['selectedID'], calculated_fields()['computer_user_details']);

	// hook: computer_user_details_after_update
	if(function_exists('computer_user_details_after_update')) {
		if($row = getRecord('computer_user_details', $data['selectedID'])) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['id'];
		$args = ['old_data' => $old_data];
		if(!computer_user_details_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update record update timestamp
	set_record_owner('computer_user_details', $selected_id);
}

function computer_user_details_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	$allowUpdate = $hasSelectedId && check_record_permission('computer_user_details', $selectedId, 'edit');
	$allowDelete = $hasSelectedId && check_record_permission('computer_user_details', $selectedId, 'delete');
	...
	if($hasSelectedId && !check_record_permission('computer_user_details', $selectedId, 'view'))
	...
	if($hasSelectedId) {
		if(!($row = getRecord('computer_user_details', $selectedId))) {
			...
			$j("#pc_id-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_pc_id__RAND__.value, t: 'computer_user_details', f: 'pc_id' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'computer_user_details', f: 'pc_id' }; },
	...
	if($dvprint) {
		$template_file = is_file("./{$templateDVP}") ? "./{$templateDVP}" : './templates/computer_user_details_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$templateDV}") ? "./{$templateDV}" : './templates/computer_user_details_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}

	// process form title
	$templateCode = str_replace('<%%DETAIL_VIEW_TITLE%%>', 'Computer user detail details', $templateCode);
	...
	$rdata = $jdata = get_defaults('computer_user_details');
	if($hasSelectedId) {
		$jdata = get_joined_record('computer_user_details', $selectedId);
		if($jdata === false) $jdata = get_defaults('computer_user_details');
		$rdata = $row;
	}
	$templateCode .= loadView('computer_user_details-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);

	// hook: computer_user_details_dv
	if(function_exists('computer_user_details_dv')) {
		$args = [];
		computer_user_details_dv(($hasSelectedId ? $selectedId : FALSE), getMemberInfo(), $templateCode, $args);
	}
