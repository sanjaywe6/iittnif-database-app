...
// Download AppGini for free from https://bigprof.com/appgini/download/
	...
	$x->QueryFieldsTV = [
		"`event_decision_table`.`decision_id`" => "decision_id",
		"IF(    CHAR_LENGTH(`outcomes_expected_table1`.`outcomes_expected_str`), CONCAT_WS('',   `outcomes_expected_table1`.`outcomes_expected_str`), '') /* Expected Outcomes of Meeting */" => "outcomes_expected_lookup",
		...
		"`event_decision_table`.`decision_str`" => "decision_str",
	...
	$x->SortFields = [
		1 => '`event_decision_table`.`decision_id`',
		2 => '`outcomes_expected_table1`.`outcomes_expected_str`',
		3 => 3,
	...
	$x->QueryFieldsFilters = [
		"`event_decision_table`.`decision_id`" => "ID",
		"IF(    CHAR_LENGTH(`outcomes_expected_table1`.`outcomes_expected_str`), CONCAT_WS('',   `outcomes_expected_table1`.`outcomes_expected_str`), '') /* Expected Outcomes of Meeting */" => "Expected Outcomes of Meeting",
		...
		"`event_decision_table`.`decision_str`" => "Decision str",
	...
	$x->QueryFrom = "`event_decision_table` LEFT JOIN `outcomes_expected_table` as outcomes_expected_table1 ON `outcomes_expected_table1`.`outcomes_expected_id`=`event_decision_table`.`outcomes_expected_lookup` LEFT JOIN `user_table` as user_table1 ON `user_table1`.`user_id`=`event_decision_table`.`decision_actor` ";
	$x->QueryWhere = '';
	$x->QueryOrder = '';
	...
	$x->PrimaryKey = '`event_decision_table`.`decision_id`';
	$x->DefaultSortField = '1';
	$x->DefaultSortDirection = 'desc';

	$x->ColWidth = [150, 150, 150, 150, 150, 150, 150, 150, 150, 150, 150, 150, ];
	$x->ColCaption = ['ID', 'Expected Outcomes of Meeting', 'Decision description', 'Decision actor', 'Action taken with date', 'Decision status', 'Decision status update date', 'Decision status remarks by superior', 'Created by', 'Created at', 'Last updated by', 'Last updated at', ];
	$x->ColFieldName = ['decision_id', 'outcomes_expected_lookup', 'decision_description', 'decision_actor', 'action_taken_with_date', 'decision_status', 'decision_status_update_date', 'decision_status_remarks_by_superior', 'created_by', 'created_at', 'last_updated_by', 'last_updated_at', ];
