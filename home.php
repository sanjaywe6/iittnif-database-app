...
<?php
	...
	if(is_array($arrTables) && count($arrTables)) {
		...
		foreach($tg as $tn => $tgroup) {
			...
			$tChkHL = array_search($tn, ['user_table','car_usage_table','cycle_usage_table','outcomes_expected_table','event_decision_table','agenda_table','decision_table','participants_table','action_actor','mou_company_area_details_table','goal_progress_table','subtask_setting_table','star_pnt','asset_allotment_table','sub_asset_allotment_table','it_inventory_billing_details','it_inventory_allotment_table','computer_usage_table','employees_appraisal_table','employees_appraisal_feedback_table','navavishkar_stay_payment_table','evaluation_table','problem_statement_table','evaluators_table','approval_billing_table','selected_proposals_final_tdp','stage_wise_budget_table_tdp','first_level_shortlisted_proposals_tdp','budget_table_tdp','panel_comments_tdp','selected_tdp','address_tdp','project_details_tdp']);
