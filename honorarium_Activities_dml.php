...
// Data functions (insert, update, delete, form) for table honorarium_Activities

// This script and data application was generated by AppGini, https://bigprof.com/appgini
// Download AppGini for free from https://bigprof.com/appgini/download/

function honorarium_Activities_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('honorarium_Activities');
	if(!$arrPerm['insert']) {
	...
	$recID = tableInsert('honorarium_Activities', $data, $recordOwner, $error_message);

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID')) && $recID !== false)
		honorarium_Activities_copy_children($recID, Request::val('SelectedID'));

	return $recID;
}

function honorarium_Activities_copy_children($destination_id, $source_id) {
	global $Translation;
...
function honorarium_Activities_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('honorarium_Activities', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: honorarium_Activities_before_delete
	if(function_exists('honorarium_Activities_before_delete')) {
		$args = [];
		if(!honorarium_Activities_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	sql("DELETE FROM `honorarium_Activities` WHERE `id`='{$selected_id}'", $eo);

	// hook: honorarium_Activities_after_delete
	if(function_exists('honorarium_Activities_after_delete')) {
		$args = [];
		honorarium_Activities_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='honorarium_Activities' AND `pkValue`='{$selected_id}'", $eo);
}

function honorarium_Activities_update(&$selected_id, &$error_message = '') {
	global $Translation;
	...
	// get existing values
	$old_data = getRecord('honorarium_Activities', $selected_id);
	...
	// hook: honorarium_Activities_before_update
	if(function_exists('honorarium_Activities_before_update')) {
		$args = ['old_data' => $old_data];
		if(!honorarium_Activities_before_update($data, getMemberInfo(), $args)) {
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="honorarium_Activities_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}


	update_calc_fields('honorarium_Activities', $data['selectedID'], calculated_fields()['honorarium_Activities']);

	// hook: honorarium_Activities_after_update
	if(function_exists('honorarium_Activities_after_update')) {
		if($row = getRecord('honorarium_Activities', $data['selectedID'])) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['id'];
		$args = ['old_data' => $old_data];
		if(!honorarium_Activities_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update record update timestamp
	set_record_owner('honorarium_Activities', $selected_id);
}

function honorarium_Activities_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	$allowUpdate = $hasSelectedId && check_record_permission('honorarium_Activities', $selectedId, 'edit');
	$allowDelete = $hasSelectedId && check_record_permission('honorarium_Activities', $selectedId, 'delete');
	...
	if($hasSelectedId && !check_record_permission('honorarium_Activities', $selectedId, 'view'))
	...
	$combo_date->MinYear = defined('honorarium_Activities.date.MinYear') ? constant('honorarium_Activities.date.MinYear') : 1900;
	$combo_date->MaxYear = defined('honorarium_Activities.date.MaxYear') ? constant('honorarium_Activities.date.MaxYear') : 2100;
	...
	if($dvprint) {
		$template_file = is_file("./{$templateDVP}") ? "./{$templateDVP}" : './templates/honorarium_Activities_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$templateDV}") ? "./{$templateDV}" : './templates/honorarium_Activities_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}

	// process form title
	$templateCode = str_replace('<%%DETAIL_VIEW_TITLE%%>', 'Honorarium Activitie details', $templateCode);
	...
	$templateCode = str_replace('<%%UPLOADFILE(honorarium_details)%%>', '', $templateCode);
	...
	if($hasSelectedId) {
		...
		$templateCode = str_replace('<%%VALUE(honorarium_details)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(honorarium_details)%%>', urlencode(''), $templateCode);
	...
	$rdata = $jdata = get_defaults('honorarium_Activities');
	if($hasSelectedId) {
		$jdata = get_joined_record('honorarium_Activities', $selectedId);
		if($jdata === false) $jdata = get_defaults('honorarium_Activities');
		$rdata = $row;
	}
	$templateCode .= loadView('honorarium_Activities-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);

	// hook: honorarium_Activities_dv
	if(function_exists('honorarium_Activities_dv')) {
		$args = [];
		honorarium_Activities_dv(($hasSelectedId ? $selectedId : FALSE), getMemberInfo(), $templateCode, $args);
	}
