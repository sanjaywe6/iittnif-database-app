...
function leave_table_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	sql("DELETE FROM `leave_table` WHERE `leave_id`='{$selected_id}'", $eo);
...
function leave_table_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
			...
			$j("#approved_by-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_approved_by__RAND__.value, t: 'leave_table', f: 'approved_by' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'leave_table', f: 'approved_by' }; },
	...
	if(!$fieldsAreEditable) {
		$jsReadOnly = '';
		$jsReadOnly .= "\tjQuery('#leave_type').replaceWith('<div class=\"form-control-static\" id=\"leave_type\">' + (jQuery('#leave_type').val() || '') + '</div>'); jQuery('#leave_type-multi-selection-help').hide();\n";
		$jsReadOnly .= "\tjQuery('#purpose_of_leave').replaceWith('<div class=\"form-control-static\" id=\"purpose_of_leave\">' + (jQuery('#purpose_of_leave').val() || '') + '</div>');\n";
	...
	// process images
	$templateCode = str_replace('<%%UPLOADFILE(leave_id)%%>', '', $templateCode);
	...
	// process values
	if($hasSelectedId) {
		$templateCode = str_replace('<%%VALUE(leave_id)%%>', safe_html($urow['leave_id']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(leave_id)%%>', urlencode($urow['leave_id']), $templateCode);
		...
		$templateCode = str_replace('<%%VALUE(leave_id)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(leave_id)%%>', urlencode(''), $templateCode);
