...
// Data functions (insert, update, delete, form) for table outcomes_expected_table

// This script and data application was generated by AppGini, https://bigprof.com/appgini
// Download AppGini for free from https://bigprof.com/appgini/download/

function outcomes_expected_table_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('outcomes_expected_table');
	if(!$arrPerm['insert']) {
	...
	$recID = tableInsert('outcomes_expected_table', $data, $recordOwner, $error_message);

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID')) && $recID !== false)
		outcomes_expected_table_copy_children($recID, Request::val('SelectedID'));

	return $recID;
}

function outcomes_expected_table_copy_children($destination_id, $source_id) {
	global $Translation;
...
function outcomes_expected_table_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('outcomes_expected_table', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: outcomes_expected_table_before_delete
	if(function_exists('outcomes_expected_table_before_delete')) {
		$args = [];
		if(!outcomes_expected_table_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	if(function_exists('outcomes_expected_table_after_delete')) {
		$args = [];
		outcomes_expected_table_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='outcomes_expected_table' AND `pkValue`='{$selected_id}'", $eo);
}

function outcomes_expected_table_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('outcomes_expected_table', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('outcomes_expected_table', $selected_id);
	...
	// hook: outcomes_expected_table_before_update
	if(function_exists('outcomes_expected_table_before_update')) {
		$args = ['old_data' => $old_data];
		if(!outcomes_expected_table_before_update($data, getMemberInfo(), $args)) {
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="outcomes_expected_table_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}


	update_calc_fields('outcomes_expected_table', $data['selectedID'], calculated_fields()['outcomes_expected_table']);

	// hook: outcomes_expected_table_after_update
	if(function_exists('outcomes_expected_table_after_update')) {
		if($row = getRecord('outcomes_expected_table', $data['selectedID'])) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['outcomes_expected_id'];
		$args = ['old_data' => $old_data];
		if(!outcomes_expected_table_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update record update timestamp
	set_record_owner('outcomes_expected_table', $selected_id);
}

function outcomes_expected_table_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	$allowUpdate = $hasSelectedId && check_record_permission('outcomes_expected_table', $selectedId, 'edit');
	$allowDelete = $hasSelectedId && check_record_permission('outcomes_expected_table', $selectedId, 'delete');
	...
	if($hasSelectedId && !check_record_permission('outcomes_expected_table', $selectedId, 'view'))
	...
	if($hasSelectedId) {
		if(!($row = getRecord('outcomes_expected_table', $selectedId))) {
			return error_message($Translation['No records found'], 'outcomes_expected_table_view.php', false);
		}
			...
			$j("#event_lookup-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_event_lookup__RAND__.value, t: 'outcomes_expected_table', f: 'event_lookup' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'outcomes_expected_table', f: 'event_lookup' }; },
	...
	if($dvprint) {
		$template_file = is_file("./{$templateDVP}") ? "./{$templateDVP}" : './templates/outcomes_expected_table_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$templateDV}") ? "./{$templateDV}" : './templates/outcomes_expected_table_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	$rdata = $jdata = get_defaults('outcomes_expected_table');
	if($hasSelectedId) {
		$jdata = get_joined_record('outcomes_expected_table', $selectedId);
		if($jdata === false) $jdata = get_defaults('outcomes_expected_table');
		$rdata = $row;
	}
	$templateCode .= loadView('outcomes_expected_table-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);

	// hook: outcomes_expected_table_dv
	if(function_exists('outcomes_expected_table_dv')) {
		$args = [];
		outcomes_expected_table_dv(($hasSelectedId ? $selectedId : FALSE), getMemberInfo(), $templateCode, $args);
	}
