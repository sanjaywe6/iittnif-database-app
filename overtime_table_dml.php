...
// Data functions (insert, update, delete, form) for table overtime_table

// This script and data application was generated by AppGini, https://bigprof.com/appgini
// Download AppGini for free from https://bigprof.com/appgini/download/

function overtime_table_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('overtime_table');
	if(!$arrPerm['insert']) {
	...
	$recID = tableInsert('overtime_table', $data, $recordOwner, $error_message);

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID')) && $recID !== false)
		overtime_table_copy_children($recID, Request::val('SelectedID'));

	return $recID;
}

function overtime_table_copy_children($destination_id, $source_id) {
	global $Translation;
...
function overtime_table_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('overtime_table', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: overtime_table_before_delete
	if(function_exists('overtime_table_before_delete')) {
		$args = [];
		if(!overtime_table_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	sql("DELETE FROM `overtime_table` WHERE `id`='{$selected_id}'", $eo);

	// hook: overtime_table_after_delete
	if(function_exists('overtime_table_after_delete')) {
		$args = [];
		overtime_table_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='overtime_table' AND `pkValue`='{$selected_id}'", $eo);
}

function overtime_table_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('overtime_table', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('overtime_table', $selected_id);
	...
	// hook: overtime_table_before_update
	if(function_exists('overtime_table_before_update')) {
		$args = ['old_data' => $old_data];
		if(!overtime_table_before_update($data, getMemberInfo(), $args)) {
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="overtime_table_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}


	update_calc_fields('overtime_table', $data['selectedID'], calculated_fields()['overtime_table']);

	// hook: overtime_table_after_update
	if(function_exists('overtime_table_after_update')) {
		if($row = getRecord('overtime_table', $data['selectedID'])) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['id'];
		$args = ['old_data' => $old_data];
		if(!overtime_table_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update record update timestamp
	set_record_owner('overtime_table', $selected_id);
}

function overtime_table_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	$allowUpdate = $hasSelectedId && check_record_permission('overtime_table', $selectedId, 'edit');
	$allowDelete = $hasSelectedId && check_record_permission('overtime_table', $selectedId, 'delete');
	...
	if($hasSelectedId && !check_record_permission('overtime_table', $selectedId, 'view'))
	...
	if(is_file(__DIR__ . '/hooks/overtime_table.approval_status.csv')) {
		$approval_status_data = addslashes(implode('', @file(__DIR__ . '/hooks/overtime_table.approval_status.csv')));
	...
	if($hasSelectedId) {
		if(!($row = getRecord('overtime_table', $selectedId))) {
			return error_message($Translation['No records found'], 'overtime_table_view.php', false);
		}
			...
			$j("#select_employee-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_select_employee__RAND__.value, t: 'overtime_table', f: 'select_employee' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'overtime_table', f: 'select_employee' }; },
	...
	if($dvprint) {
		$template_file = is_file("./{$templateDVP}") ? "./{$templateDVP}" : './templates/overtime_table_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$templateDV}") ? "./{$templateDV}" : './templates/overtime_table_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	$rdata = $jdata = get_defaults('overtime_table');
	if($hasSelectedId) {
		$jdata = get_joined_record('overtime_table', $selectedId);
		if($jdata === false) $jdata = get_defaults('overtime_table');
		$rdata = $row;
	}
	$templateCode .= loadView('overtime_table-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);

	// hook: overtime_table_dv
	if(function_exists('overtime_table_dv')) {
		$args = [];
		overtime_table_dv(($hasSelectedId ? $selectedId : FALSE), getMemberInfo(), $templateCode, $args);
	}
