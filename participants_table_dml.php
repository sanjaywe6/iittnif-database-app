...
// Data functions (insert, update, delete, form) for table participants_table

// This script and data application was generated by AppGini, https://bigprof.com/appgini
// Download AppGini for free from https://bigprof.com/appgini/download/

function participants_table_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('participants_table');
	if(!$arrPerm['insert']) {
	...
	$recID = tableInsert('participants_table', $data, $recordOwner, $error_message);

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID')) && $recID !== false)
		participants_table_copy_children($recID, Request::val('SelectedID'));

	return $recID;
}

function participants_table_copy_children($destination_id, $source_id) {
	global $Translation;
...
function participants_table_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('participants_table', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: participants_table_before_delete
	if(function_exists('participants_table_before_delete')) {
		$args = [];
		if(!participants_table_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	sql("DELETE FROM `participants_table` WHERE `participants_id`='{$selected_id}'", $eo);

	// hook: participants_table_after_delete
	if(function_exists('participants_table_after_delete')) {
		$args = [];
		participants_table_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='participants_table' AND `pkValue`='{$selected_id}'", $eo);
}

function participants_table_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('participants_table', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('participants_table', $selected_id);
	...
	// hook: participants_table_before_update
	if(function_exists('participants_table_before_update')) {
		$args = ['old_data' => $old_data];
		if(!participants_table_before_update($data, getMemberInfo(), $args)) {
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="participants_table_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}


	update_calc_fields('participants_table', $data['selectedID'], calculated_fields()['participants_table']);

	// hook: participants_table_after_update
	if(function_exists('participants_table_after_update')) {
		if($row = getRecord('participants_table', $data['selectedID'])) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['participants_id'];
		$args = ['old_data' => $old_data];
		if(!participants_table_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update record update timestamp
	set_record_owner('participants_table', $selected_id);
}

function participants_table_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	$allowUpdate = $hasSelectedId && check_record_permission('participants_table', $selectedId, 'edit');
	$allowDelete = $hasSelectedId && check_record_permission('participants_table', $selectedId, 'delete');
	...
	if($hasSelectedId && !check_record_permission('participants_table', $selectedId, 'view'))
	...
	if(is_file(__DIR__ . '/hooks/participants_table.participant_type.csv')) {
		$participant_type_data = addslashes(implode('', @file(__DIR__ . '/hooks/participants_table.participant_type.csv')));
	...
	if(is_file(__DIR__ . '/hooks/participants_table.accepted_status.csv')) {
		$accepted_status_data = addslashes(implode('', @file(__DIR__ . '/hooks/participants_table.accepted_status.csv')));
	...
	$combo_status_date->MinYear = defined('participants_table.status_date.MinYear') ? constant('participants_table.status_date.MinYear') : 1900;
	$combo_status_date->MaxYear = defined('participants_table.status_date.MaxYear') ? constant('participants_table.status_date.MaxYear') : 2100;
	...
	if($hasSelectedId) {
		if(!($row = getRecord('participants_table', $selectedId))) {
			return error_message($Translation['No records found'], 'participants_table_view.php', false);
		}
			...
			$j("#event_lookup-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_event_lookup__RAND__.value, t: 'participants_table', f: 'event_lookup' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'participants_table', f: 'event_lookup' }; },
			...
			$j("#meeting_lookup-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_meeting_lookup__RAND__.value, t: 'participants_table', f: 'meeting_lookup' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'participants_table', f: 'meeting_lookup' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
	...
	if($dvprint) {
		$template_file = is_file("./{$templateDVP}") ? "./{$templateDVP}" : './templates/participants_table_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$templateDV}") ? "./{$templateDV}" : './templates/participants_table_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	$lookup_fields = ['event_lookup' => ['event_table', 'Event'], 'meeting_lookup' => ['meetings_table', 'Meeting'], ];
	...
	$rdata = $jdata = get_defaults('participants_table');
	if($hasSelectedId) {
		$jdata = get_joined_record('participants_table', $selectedId);
		if($jdata === false) $jdata = get_defaults('participants_table');
		$rdata = $row;
	}
	$templateCode .= loadView('participants_table-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);

	// hook: participants_table_dv
	if(function_exists('participants_table_dv')) {
		$args = [];
		participants_table_dv(($hasSelectedId ? $selectedId : FALSE), getMemberInfo(), $templateCode, $args);
	}
