...
// Data functions (insert, update, delete, form) for table publications_and_intellectual_activities

// This script and data application was generated by AppGini, https://bigprof.com/appgini
// Download AppGini for free from https://bigprof.com/appgini/download/

function publications_and_intellectual_activities_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('publications_and_intellectual_activities');
	if(!$arrPerm['insert']) {
	...
	$recID = tableInsert('publications_and_intellectual_activities', $data, $recordOwner, $error_message);

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID')) && $recID !== false)
		publications_and_intellectual_activities_copy_children($recID, Request::val('SelectedID'));

	return $recID;
}

function publications_and_intellectual_activities_copy_children($destination_id, $source_id) {
	global $Translation;
...
function publications_and_intellectual_activities_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('publications_and_intellectual_activities', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: publications_and_intellectual_activities_before_delete
	if(function_exists('publications_and_intellectual_activities_before_delete')) {
		$args = [];
		if(!publications_and_intellectual_activities_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	if(function_exists('publications_and_intellectual_activities_after_delete')) {
		$args = [];
		publications_and_intellectual_activities_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='publications_and_intellectual_activities' AND `pkValue`='{$selected_id}'", $eo);
}

function publications_and_intellectual_activities_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('publications_and_intellectual_activities', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('publications_and_intellectual_activities', $selected_id);
	...
	// hook: publications_and_intellectual_activities_before_update
	if(function_exists('publications_and_intellectual_activities_before_update')) {
		$args = ['old_data' => $old_data];
		if(!publications_and_intellectual_activities_before_update($data, getMemberInfo(), $args)) {
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="publications_and_intellectual_activities_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}


	update_calc_fields('publications_and_intellectual_activities', $data['selectedID'], calculated_fields()['publications_and_intellectual_activities']);

	// hook: publications_and_intellectual_activities_after_update
	if(function_exists('publications_and_intellectual_activities_after_update')) {
		if($row = getRecord('publications_and_intellectual_activities', $data['selectedID'])) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['id'];
		$args = ['old_data' => $old_data];
		if(!publications_and_intellectual_activities_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update record update timestamp
	set_record_owner('publications_and_intellectual_activities', $selected_id);
}

function publications_and_intellectual_activities_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	$allowUpdate = $hasSelectedId && check_record_permission('publications_and_intellectual_activities', $selectedId, 'edit');
	$allowDelete = $hasSelectedId && check_record_permission('publications_and_intellectual_activities', $selectedId, 'delete');
	...
	if($hasSelectedId && !check_record_permission('publications_and_intellectual_activities', $selectedId, 'view'))
	...
	if(is_file(__DIR__ . '/hooks/publications_and_intellectual_activities.year.csv')) {
		$year_data = addslashes(implode('', @file(__DIR__ . '/hooks/publications_and_intellectual_activities.year.csv')));
	...
	if($dvprint) {
		$template_file = is_file("./{$templateDVP}") ? "./{$templateDVP}" : './templates/publications_and_intellectual_activities_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$templateDV}") ? "./{$templateDV}" : './templates/publications_and_intellectual_activities_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	$rdata = $jdata = get_defaults('publications_and_intellectual_activities');
	if($hasSelectedId) {
		$jdata = get_joined_record('publications_and_intellectual_activities', $selectedId);
		if($jdata === false) $jdata = get_defaults('publications_and_intellectual_activities');
		$rdata = $row;
	}
	$templateCode .= loadView('publications_and_intellectual_activities-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);

	// hook: publications_and_intellectual_activities_dv
	if(function_exists('publications_and_intellectual_activities_dv')) {
		$args = [];
		publications_and_intellectual_activities_dv(($hasSelectedId ? $selectedId : FALSE), getMemberInfo(), $templateCode, $args);
	}
