...
// Data functions (insert, update, delete, form) for table sub_asset_allotment_table

// This script and data application was generated by AppGini, https://bigprof.com/appgini
// Download AppGini for free from https://bigprof.com/appgini/download/

function sub_asset_allotment_table_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('sub_asset_allotment_table');
	if(!$arrPerm['insert']) {
		$error_message = $Translation['no insert permission'];
		return false;
	}

	// automatic sub_asset_lookup if passed as filterer
	if(Request::val('filterer_sub_asset_lookup')) {
		$_REQUEST['sub_asset_lookup'] = Request::val('filterer_sub_asset_lookup');
	}
	...
	// automatic sub_asset_lookup if passed as filterer
	if(Request::val('filterer_sub_asset_lookup')) {
		$data['sub_asset_lookup'] = Request::val('filterer_sub_asset_lookup');
	}
	// record owner is current user
	$recordOwner = getLoggedMemberID();

	$recID = tableInsert('sub_asset_allotment_table', $data, $recordOwner, $error_message);

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID')) && $recID !== false)
		sub_asset_allotment_table_copy_children($recID, Request::val('SelectedID'));

	return $recID;
}

function sub_asset_allotment_table_copy_children($destination_id, $source_id) {
	global $Translation;
...
function sub_asset_allotment_table_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('sub_asset_allotment_table', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: sub_asset_allotment_table_before_delete
	if(function_exists('sub_asset_allotment_table_before_delete')) {
		$args = [];
		if(!sub_asset_allotment_table_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	sql("DELETE FROM `sub_asset_allotment_table` WHERE `id`='{$selected_id}'", $eo);

	// hook: sub_asset_allotment_table_after_delete
	if(function_exists('sub_asset_allotment_table_after_delete')) {
		$args = [];
		sub_asset_allotment_table_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='sub_asset_allotment_table' AND `pkValue`='{$selected_id}'", $eo);
}

function sub_asset_allotment_table_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('sub_asset_allotment_table', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('sub_asset_allotment_table', $selected_id);
	...
	// hook: sub_asset_allotment_table_before_update
	if(function_exists('sub_asset_allotment_table_before_update')) {
		$args = ['old_data' => $old_data];
		if(!sub_asset_allotment_table_before_update($data, getMemberInfo(), $args)) {
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="sub_asset_allotment_table_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}


	update_calc_fields('sub_asset_allotment_table', $data['selectedID'], calculated_fields()['sub_asset_allotment_table']);

	// hook: sub_asset_allotment_table_after_update
	if(function_exists('sub_asset_allotment_table_after_update')) {
		if($row = getRecord('sub_asset_allotment_table', $data['selectedID'])) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['id'];
		$args = ['old_data' => $old_data];
		if(!sub_asset_allotment_table_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update record update timestamp
	set_record_owner('sub_asset_allotment_table', $selected_id);
}

function sub_asset_allotment_table_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	$allowUpdate = $hasSelectedId && check_record_permission('sub_asset_allotment_table', $selectedId, 'edit');
	$allowDelete = $hasSelectedId && check_record_permission('sub_asset_allotment_table', $selectedId, 'delete');
	...
	if($hasSelectedId && !check_record_permission('sub_asset_allotment_table', $selectedId, 'view'))
	...
	$filterer_sub_asset_lookup = Request::val('filterer_sub_asset_lookup');
	...
	// combobox: sub_asset_lookup
	$combo_sub_asset_lookup = new DataCombo;
	// combobox: select_employee
	$combo_select_employee = new DataCombo;
	// combobox: date
	$combo_date = new DateCombo;
	$combo_date->DateFormat = "dmy";
	$combo_date->MinYear = defined('sub_asset_allotment_table.date.MinYear') ? constant('sub_asset_allotment_table.date.MinYear') : 1900;
	$combo_date->MaxYear = defined('sub_asset_allotment_table.date.MaxYear') ? constant('sub_asset_allotment_table.date.MaxYear') : 2100;
	...
	if(is_file(__DIR__ . '/hooks/sub_asset_allotment_table.status.csv')) {
		$status_data = addslashes(implode('', @file(__DIR__ . '/hooks/sub_asset_allotment_table.status.csv')));
	...
	$combo_returned_date->MinYear = defined('sub_asset_allotment_table.returned_date.MinYear') ? constant('sub_asset_allotment_table.returned_date.MinYear') : 1900;
	$combo_returned_date->MaxYear = defined('sub_asset_allotment_table.returned_date.MaxYear') ? constant('sub_asset_allotment_table.returned_date.MaxYear') : 2100;
	...
	if($hasSelectedId) {
		if(!($row = getRecord('sub_asset_allotment_table', $selectedId))) {
			return error_message($Translation['No records found'], 'sub_asset_allotment_table_view.php', false);
		}
		$combo_sub_asset_lookup->SelectedData = $row['sub_asset_lookup'];
		...
		$combo_sub_asset_lookup->SelectedData = $filterer_sub_asset_lookup;
	...
	$combo_sub_asset_lookup->HTML = '<span id="sub_asset_lookup-container' . $rnd1 . '"></span><input type="hidden" name="sub_asset_lookup" id="sub_asset_lookup' . $rnd1 . '" value="' . html_attr($combo_sub_asset_lookup->SelectedData) . '">';
	$combo_sub_asset_lookup->MatchText = '<span id="sub_asset_lookup-container-readonly' . $rnd1 . '"></span><input type="hidden" name="sub_asset_lookup" id="sub_asset_lookup' . $rnd1 . '" value="' . html_attr($combo_sub_asset_lookup->SelectedData) . '">';
	...
	<script>
		...
		AppGini.current_sub_asset_lookup__RAND__ = { text: "", value: "<?php echo addslashes($hasSelectedId ? $urow['sub_asset_lookup'] : htmlspecialchars($filterer_sub_asset_lookup, ENT_QUOTES)); ?>"};
		...
		$j(function() {
			setTimeout(function() {
				if(typeof(sub_asset_lookup_reload__RAND__) == 'function') sub_asset_lookup_reload__RAND__();
		...
		function sub_asset_lookup_reload__RAND__() {
		<?php if($fieldsAreEditable) { ?>

			$j("#sub_asset_lookup-container__RAND__").select2({
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'sub_asset_allotment_table', f: 'sub_asset_lookup' }; },
			...
			if(!$j("#sub_asset_lookup-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_sub_asset_lookup__RAND__.value, t: 'sub_asset_allotment_table', f: 'sub_asset_lookup' },
			...
			$j("#select_employee-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_select_employee__RAND__.value, t: 'sub_asset_allotment_table', f: 'select_employee' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'sub_asset_allotment_table', f: 'select_employee' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			$j("#alloted_by-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_alloted_by__RAND__.value, t: 'sub_asset_allotment_table', f: 'alloted_by' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'sub_asset_allotment_table', f: 'alloted_by' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
	...
	if($dvprint) {
		$template_file = is_file("./{$templateDVP}") ? "./{$templateDVP}" : './templates/sub_asset_allotment_table_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$templateDV}") ? "./{$templateDV}" : './templates/sub_asset_allotment_table_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process combos
	$templateCode = str_replace('<%%COMBO(sub_asset_lookup)%%>', $combo_sub_asset_lookup->HTML, $templateCode);
	$templateCode = str_replace('<%%COMBOTEXT(sub_asset_lookup)%%>', $combo_sub_asset_lookup->MatchText, $templateCode);
	$templateCode = str_replace('<%%URLCOMBOTEXT(sub_asset_lookup)%%>', urlencode($combo_sub_asset_lookup->MatchText), $templateCode);
	...
	$templateCode = str_replace('<%%UPLOADFILE(sub_asset_lookup)%%>', '', $templateCode);
	...
	if($hasSelectedId) {
		...
		$templateCode = str_replace('<%%VALUE(sub_asset_lookup)%%>', safe_html($urow['sub_asset_lookup']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(sub_asset_lookup)%%>', urlencode($urow['sub_asset_lookup']), $templateCode);
		...
		$templateCode = str_replace('<%%VALUE(sub_asset_lookup)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(sub_asset_lookup)%%>', urlencode(''), $templateCode);
	...
	if(isset($filterField[1]) && $filterField[1] == '3' && $filterOperator[1] == '<=>')
		$templateCode.="\n<input type=hidden name=sub_asset_lookup value=\"" . html_attr($filterValue[1]) . "\">\n";
	...
	$rdata = $jdata = get_defaults('sub_asset_allotment_table');
	if($hasSelectedId) {
		$jdata = get_joined_record('sub_asset_allotment_table', $selectedId);
		if($jdata === false) $jdata = get_defaults('sub_asset_allotment_table');
		$rdata = $row;
	}
	$templateCode .= loadView('sub_asset_allotment_table-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);

	// hook: sub_asset_allotment_table_dv
	if(function_exists('sub_asset_allotment_table_dv')) {
		$args = [];
		sub_asset_allotment_table_dv(($hasSelectedId ? $selectedId : FALSE), getMemberInfo(), $templateCode, $args);
	}
