...
// Data functions (insert, update, delete, form) for table sub_asset_table

// This script and data application was generated by AppGini, https://bigprof.com/appgini
// Download AppGini for free from https://bigprof.com/appgini/download/

function sub_asset_table_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('sub_asset_table');
	if(!$arrPerm['insert']) {
	...
	$data = [
		...
		'CustodianSignature' => Request::fileUpload('CustodianSignature', [
			...
			'failure' => function($selected_id, $fileRemoved) {
				...
				return existing_value('sub_asset_table', 'CustodianSignature', Request::val('SelectedID'));
			},
		]),
	...
	$recID = tableInsert('sub_asset_table', $data, $recordOwner, $error_message);

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID')) && $recID !== false)
		sub_asset_table_copy_children($recID, Request::val('SelectedID'));

	return $recID;
}

function sub_asset_table_copy_children($destination_id, $source_id) {
	global $Translation;
...
function sub_asset_table_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('sub_asset_table', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: sub_asset_table_before_delete
	if(function_exists('sub_asset_table_before_delete')) {
		$args = [];
		if(!sub_asset_table_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	$res = sql("SELECT `id` FROM `sub_asset_table` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `sub_asset_allotment_table` WHERE `sub_asset_lookup`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	$childrenATag = '<a class="alert-link" href="sub_asset_allotment_table_view.php?filterer_sub_asset_lookup=' . urlencode($id[0]) . '">%s</a>';
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		...
		$RetMsg = str_replace('<RelatedRecords>', sprintf($childrenATag, $rirow[0]), $RetMsg);
		$RetMsg = str_replace(['[<TableName>]', '<TableName>'], sprintf($childrenATag, 'sub_asset_allotment_table'), $RetMsg);
		$RetMsg = str_replace('<Delete>', '<input type="button" class="btn btn-danger" value="' . html_attr($Translation['yes']) . '" onClick="window.location = `sub_asset_table_view.php?SelectedID=' . urlencode($selected_id) . '&delete_x=1&confirmed=1&csrf_token=' . urlencode(csrf_token(false, true)) . (Request::val('Embedded') ? '&Embedded=1' : '') . '`;">', $RetMsg);
		$RetMsg = str_replace('<Cancel>', '<input type="button" class="btn btn-success" value="' . html_attr($Translation[ 'no']) . '" onClick="window.location = `sub_asset_table_view.php?SelectedID=' . urlencode($selected_id) . (Request::val('Embedded') ? '&Embedded=1' : '') . '`;">', $RetMsg);
		return $RetMsg;
	}

	// delete file stored in the 'CustodianSignature' field
	$res = sql("SELECT `CustodianSignature` FROM `sub_asset_table` WHERE `id`='{$selected_id}'", $eo);
	...
	sql("DELETE FROM `sub_asset_table` WHERE `id`='{$selected_id}'", $eo);

	// hook: sub_asset_table_after_delete
	if(function_exists('sub_asset_table_after_delete')) {
		$args = [];
		sub_asset_table_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='sub_asset_table' AND `pkValue`='{$selected_id}'", $eo);
}

function sub_asset_table_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('sub_asset_table', $selected_id, 'edit')) return false;
	...
	$data = [
		...
		'CustodianSignature' => Request::fileUpload('CustodianSignature', [
			...
			'remove' => function($selected_id) {
				...
				$oldFile = existing_value('sub_asset_table', 'CustodianSignature', $selected_id);
				if(!$oldFile) return;
			...
			'failure' => function($selected_id, $fileRemoved) {
				...
				return existing_value('sub_asset_table', 'CustodianSignature', $selected_id);
			},
		]),
		'remarks' => br2nl(Request::val('remarks', '')),
	...
	// get existing values
	$old_data = getRecord('sub_asset_table', $selected_id);
	...
	// hook: sub_asset_table_before_update
	if(function_exists('sub_asset_table_before_update')) {
		$args = ['old_data' => $old_data];
		if(!sub_asset_table_before_update($data, getMemberInfo(), $args)) {
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="sub_asset_table_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}


	update_calc_fields('sub_asset_table', $data['selectedID'], calculated_fields()['sub_asset_table']);

	// hook: sub_asset_table_after_update
	if(function_exists('sub_asset_table_after_update')) {
		if($row = getRecord('sub_asset_table', $data['selectedID'])) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['id'];
		$args = ['old_data' => $old_data];
		if(!sub_asset_table_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update record update timestamp
	set_record_owner('sub_asset_table', $selected_id);
}

function sub_asset_table_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	$allowUpdate = $hasSelectedId && check_record_permission('sub_asset_table', $selectedId, 'edit');
	$allowDelete = $hasSelectedId && check_record_permission('sub_asset_table', $selectedId, 'delete');
	...
	if($hasSelectedId && !check_record_permission('sub_asset_table', $selectedId, 'view'))
	...
	$combo_Date->MinYear = defined('sub_asset_table.Date.MinYear') ? constant('sub_asset_table.Date.MinYear') : 1900;
	$combo_Date->MaxYear = defined('sub_asset_table.Date.MaxYear') ? constant('sub_asset_table.Date.MaxYear') : 2100;
	...
	$combo_PODATE->MinYear = defined('sub_asset_table.PODATE.MinYear') ? constant('sub_asset_table.PODATE.MinYear') : 1900;
	$combo_PODATE->MaxYear = defined('sub_asset_table.PODATE.MaxYear') ? constant('sub_asset_table.PODATE.MaxYear') : 2100;
	...
	$combo_BillDate->MinYear = defined('sub_asset_table.BillDate.MinYear') ? constant('sub_asset_table.BillDate.MinYear') : 1900;
	$combo_BillDate->MaxYear = defined('sub_asset_table.BillDate.MaxYear') ? constant('sub_asset_table.BillDate.MaxYear') : 2100;
	...
	if($hasSelectedId) {
		if(!($row = getRecord('sub_asset_table', $selectedId))) {
			return error_message($Translation['No records found'], 'sub_asset_table_view.php', false);
		}
	...
	if($dvprint) {
		$template_file = is_file("./{$templateDVP}") ? "./{$templateDVP}" : './templates/sub_asset_table_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$templateDV}") ? "./{$templateDV}" : './templates/sub_asset_table_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	$rdata = $jdata = get_defaults('sub_asset_table');
	if($hasSelectedId) {
		$jdata = get_joined_record('sub_asset_table', $selectedId);
		if($jdata === false) $jdata = get_defaults('sub_asset_table');
		$rdata = $row;
	}
	$templateCode .= loadView('sub_asset_table-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);

	// hook: sub_asset_table_dv
	if(function_exists('sub_asset_table_dv')) {
		$args = [];
		sub_asset_table_dv(($hasSelectedId ? $selectedId : FALSE), getMemberInfo(), $templateCode, $args);
	}
