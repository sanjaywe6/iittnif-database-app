<?php
	...
	$current_table = 'it_inventory_billing_details';
			...
			<table data-tablename="<?php echo $current_table; ?>" class="table table-striped table-hover table-condensed table-bordered">
				...
				<tbody>
					...
					<tr data-id="<?php echo html_attr($pkValue); ?>">
						...
						<td class="<?php echo "{$parameters['ChildTable']}-{$config['display-field-names'][12]}"; ?>" id="<?php echo "{$parameters['ChildTable']}-{$config['display-field-names'][12]}-" . html_attr($record[$config['child-primary-key-index']]); ?>"><a href="<?php echo getUploadDir('') . urlencode($record[12]); ?>" data-lightbox="it_inventory_billing_details-image"><img src="thumbnail.php?i=<?php echo urlencode($record[12]); ?>&t=it_inventory_billing_details&f=image&v=tv" class="img-thumbnail"></a></td>
