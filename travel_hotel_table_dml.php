...
// Data functions (insert, update, delete, form) for table travel_hotel_table

// This script and data application was generated by AppGini, https://bigprof.com/appgini
// Download AppGini for free from https://bigprof.com/appgini/download/

function travel_hotel_table_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('travel_hotel_table');
	if(!$arrPerm['insert']) {
	...
	$recID = tableInsert('travel_hotel_table', $data, $recordOwner, $error_message);

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID')) && $recID !== false)
		travel_hotel_table_copy_children($recID, Request::val('SelectedID'));

	return $recID;
}

function travel_hotel_table_copy_children($destination_id, $source_id) {
	global $Translation;
...
function travel_hotel_table_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('travel_hotel_table', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: travel_hotel_table_before_delete
	if(function_exists('travel_hotel_table_before_delete')) {
		$args = [];
		if(!travel_hotel_table_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	sql("DELETE FROM `travel_hotel_table` WHERE `id`='{$selected_id}'", $eo);

	// hook: travel_hotel_table_after_delete
	if(function_exists('travel_hotel_table_after_delete')) {
		$args = [];
		travel_hotel_table_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='travel_hotel_table' AND `pkValue`='{$selected_id}'", $eo);
}

function travel_hotel_table_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('travel_hotel_table', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('travel_hotel_table', $selected_id);
	...
	// hook: travel_hotel_table_before_update
	if(function_exists('travel_hotel_table_before_update')) {
		$args = ['old_data' => $old_data];
		if(!travel_hotel_table_before_update($data, getMemberInfo(), $args)) {
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="travel_hotel_table_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}


	update_calc_fields('travel_hotel_table', $data['selectedID'], calculated_fields()['travel_hotel_table']);

	// hook: travel_hotel_table_after_update
	if(function_exists('travel_hotel_table_after_update')) {
		if($row = getRecord('travel_hotel_table', $data['selectedID'])) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['id'];
		$args = ['old_data' => $old_data];
		if(!travel_hotel_table_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update record update timestamp
	set_record_owner('travel_hotel_table', $selected_id);
}

function travel_hotel_table_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	$allowUpdate = $hasSelectedId && check_record_permission('travel_hotel_table', $selectedId, 'edit');
	$allowDelete = $hasSelectedId && check_record_permission('travel_hotel_table', $selectedId, 'delete');
	...
	if($hasSelectedId && !check_record_permission('travel_hotel_table', $selectedId, 'view'))
	...
	$combo_checkin_date->MinYear = defined('travel_hotel_table.checkin_date.MinYear') ? constant('travel_hotel_table.checkin_date.MinYear') : 1900;
	$combo_checkin_date->MaxYear = defined('travel_hotel_table.checkin_date.MaxYear') ? constant('travel_hotel_table.checkin_date.MaxYear') : 2100;
	...
	$combo_checkout_date->MinYear = defined('travel_hotel_table.checkout_date.MinYear') ? constant('travel_hotel_table.checkout_date.MinYear') : 1900;
	$combo_checkout_date->MaxYear = defined('travel_hotel_table.checkout_date.MaxYear') ? constant('travel_hotel_table.checkout_date.MaxYear') : 2100;
	...
	if($hasSelectedId) {
		if(!($row = getRecord('travel_hotel_table', $selectedId))) {
			return error_message($Translation['No records found'], 'travel_hotel_table_view.php', false);
		}
			...
			$j("#travel_details-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_travel_details__RAND__.value, t: 'travel_hotel_table', f: 'travel_details' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'travel_hotel_table', f: 'travel_details' }; },
	...
	if($dvprint) {
		$template_file = is_file("./{$templateDVP}") ? "./{$templateDVP}" : './templates/travel_hotel_table_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$templateDV}") ? "./{$templateDV}" : './templates/travel_hotel_table_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	$rdata = $jdata = get_defaults('travel_hotel_table');
	if($hasSelectedId) {
		$jdata = get_joined_record('travel_hotel_table', $selectedId);
		if($jdata === false) $jdata = get_defaults('travel_hotel_table');
		$rdata = $row;
	}
	$templateCode .= loadView('travel_hotel_table-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);

	// hook: travel_hotel_table_dv
	if(function_exists('travel_hotel_table_dv')) {
		$args = [];
		travel_hotel_table_dv(($hasSelectedId ? $selectedId : FALSE), getMemberInfo(), $templateCode, $args);
	}
