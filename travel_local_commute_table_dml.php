...
// Data functions (insert, update, delete, form) for table travel_local_commute_table

// This script and data application was generated by AppGini, https://bigprof.com/appgini
// Download AppGini for free from https://bigprof.com/appgini/download/

function travel_local_commute_table_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('travel_local_commute_table');
	if(!$arrPerm['insert']) {
	...
	$recID = tableInsert('travel_local_commute_table', $data, $recordOwner, $error_message);

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID')) && $recID !== false)
		travel_local_commute_table_copy_children($recID, Request::val('SelectedID'));

	return $recID;
}

function travel_local_commute_table_copy_children($destination_id, $source_id) {
	global $Translation;
...
function travel_local_commute_table_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('travel_local_commute_table', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: travel_local_commute_table_before_delete
	if(function_exists('travel_local_commute_table_before_delete')) {
		$args = [];
		if(!travel_local_commute_table_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	sql("DELETE FROM `travel_local_commute_table` WHERE `id`='{$selected_id}'", $eo);

	// hook: travel_local_commute_table_after_delete
	if(function_exists('travel_local_commute_table_after_delete')) {
		$args = [];
		travel_local_commute_table_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='travel_local_commute_table' AND `pkValue`='{$selected_id}'", $eo);
}

function travel_local_commute_table_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('travel_local_commute_table', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('travel_local_commute_table', $selected_id);
	...
	// hook: travel_local_commute_table_before_update
	if(function_exists('travel_local_commute_table_before_update')) {
		$args = ['old_data' => $old_data];
		if(!travel_local_commute_table_before_update($data, getMemberInfo(), $args)) {
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="travel_local_commute_table_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}


	update_calc_fields('travel_local_commute_table', $data['selectedID'], calculated_fields()['travel_local_commute_table']);

	// hook: travel_local_commute_table_after_update
	if(function_exists('travel_local_commute_table_after_update')) {
		if($row = getRecord('travel_local_commute_table', $data['selectedID'])) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['id'];
		$args = ['old_data' => $old_data];
		if(!travel_local_commute_table_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update record update timestamp
	set_record_owner('travel_local_commute_table', $selected_id);
}

function travel_local_commute_table_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	$allowUpdate = $hasSelectedId && check_record_permission('travel_local_commute_table', $selectedId, 'edit');
	$allowDelete = $hasSelectedId && check_record_permission('travel_local_commute_table', $selectedId, 'delete');
	...
	if($hasSelectedId && !check_record_permission('travel_local_commute_table', $selectedId, 'view'))
	...
	if(is_file(__DIR__ . '/hooks/travel_local_commute_table.gender.csv')) {
		$gender_data = addslashes(implode('', @file(__DIR__ . '/hooks/travel_local_commute_table.gender.csv')));
	...
	// combobox: local_commute_type
	$combo_local_commute_type = new Combo;
	$combo_local_commute_type->ListType = 0;
	$combo_local_commute_type->MultipleSeparator = ', ';
	$combo_local_commute_type->ListBoxHeight = 10;
	$combo_local_commute_type->RadiosPerLine = 1;
	if(is_file(__DIR__ . '/hooks/travel_local_commute_table.local_commute_type.csv')) {
		$local_commute_type_data = addslashes(implode('', @file(__DIR__ . '/hooks/travel_local_commute_table.local_commute_type.csv')));
		$combo_local_commute_type->ListItem = array_trim(explode('||', entitiesToUTF8(convertLegacyOptions($local_commute_type_data))));
		$combo_local_commute_type->ListData = $combo_local_commute_type->ListItem;
	} else {
		$combo_local_commute_type->ListItem = array_trim(explode('||', entitiesToUTF8(convertLegacyOptions("Cab;;Bus;;Metro"))));
		$combo_local_commute_type->ListData = $combo_local_commute_type->ListItem;
	}
	$combo_local_commute_type->SelectName = 'local_commute_type';
	...
	if(is_file(__DIR__ . '/hooks/travel_local_commute_table.approval_status.csv')) {
		$approval_status_data = addslashes(implode('', @file(__DIR__ . '/hooks/travel_local_commute_table.approval_status.csv')));
	...
	if($hasSelectedId) {
		if(!($row = getRecord('travel_local_commute_table', $selectedId))) {
			return error_message($Translation['No records found'], 'travel_local_commute_table_view.php', false);
		}
		$combo_gender->SelectedData = $row['gender'];
		$combo_local_commute_type->SelectedData = $row['local_commute_type'];
	...
	if($dvprint) {
		$template_file = is_file("./{$templateDVP}") ? "./{$templateDVP}" : './templates/travel_local_commute_table_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$templateDV}") ? "./{$templateDV}" : './templates/travel_local_commute_table_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	$templateCode = str_replace('<%%COMBO(local_commute_type)%%>', $combo_local_commute_type->HTML, $templateCode);
	$templateCode = str_replace('<%%COMBOTEXT(local_commute_type)%%>', $combo_local_commute_type->SelectedData, $templateCode);
	...
	$templateCode = str_replace('<%%UPLOADFILE(local_commute_type)%%>', '', $templateCode);
	...
	if($hasSelectedId) {
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(local_commute_type)%%>', safe_html($urow['local_commute_type']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(local_commute_type)%%>', html_attr($row['local_commute_type']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(local_commute_type)%%>', urlencode($urow['local_commute_type']), $templateCode);
		...
		$templateCode = str_replace('<%%VALUE(local_commute_type)%%>', 'Cab', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(local_commute_type)%%>', urlencode('Cab'), $templateCode);
	...
	$rdata = $jdata = get_defaults('travel_local_commute_table');
	if($hasSelectedId) {
		$jdata = get_joined_record('travel_local_commute_table', $selectedId);
		if($jdata === false) $jdata = get_defaults('travel_local_commute_table');
		$rdata = $row;
	}
	$templateCode .= loadView('travel_local_commute_table-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);

	// hook: travel_local_commute_table_dv
	if(function_exists('travel_local_commute_table_dv')) {
		$args = [];
		travel_local_commute_table_dv(($hasSelectedId ? $selectedId : FALSE), getMemberInfo(), $templateCode, $args);
	}
